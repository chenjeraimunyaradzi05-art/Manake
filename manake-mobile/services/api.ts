...
function toPaginatedResponse<T>(value: unknown): PaginatedResponse<T> {
  if (value && typeof value === 'object' && 'success' in value && 'data' in value && 'pagination' in value) {
...
function normalizeStory(raw: any): Story {
  ...
  const createdAt = raw?.publishedAt ?? raw?.date ?? raw?.createdAt ?? new Date().toISOString();
...
export const authApi = {
  ...
  socialLoginGoogle: async (payload: { idToken: string; redirectUri?: string }): Promise<ApiResponse<{ user: User; token: string }>> => {
  ...
  socialLoginApple: async (payload: { code: string; codeVerifier?: string; redirectUri?: string; clientId?: string }): Promise<ApiResponse<{ user: User; token: string }>> => {
...
export const storiesApi = {
  ...
  getFeatured: async (): Promise<ApiResponse<Story[]>> => {
    ...
    const raw = await fetchApi<unknown>('/stories?featured=true&page=1&limit=10');
    const normalized = toPaginatedResponse<any>(raw);
    return { success: true, data: normalized.data.map((s: any) => normalizeStory(s)) };
  },
  ...
  getByCategory: async (category: string, page = 1): Promise<PaginatedResponse<Story>> => {
    const serverCategory = toServerCategory(category);
    const raw = await fetchApi<unknown>(`/stories?category=${encodeURIComponent(serverCategory)}&page=${page}&limit=10`);
    const normalized = toPaginatedResponse<any>(raw);
    return {
      ...normalized,
      data: normalized.data.map((s: any) => normalizeStory(s)),
  ...
  likeStory: async (id: string): Promise<ApiResponse<{ likes: number }>> => {
    const raw = await fetchApi<unknown>(`/stories/${id}/like`, { method: 'POST' });
    return toApiResponse(raw);
  },

  unlikeStory: async (id: string): Promise<ApiResponse<{ likes: number }>> => {
    const raw = await fetchApi<unknown>(`/stories/${id}/unlike`, { method: 'POST' });
    return toApiResponse(raw);
  },

  addComment: async (id: string, args: { author: string; content: string }): Promise<ApiResponse<StoryComment>> => {
  ...
  getComments: async (id: string): Promise<ApiResponse<StoryComment[]>> => {
    ...
    if (raw && typeof raw === 'object' && 'data' in raw && !('success' in raw)) {
      ...
      return { success: true, data: items.map((c: any) => normalizeStoryComment(id, c)) };
    }
    return toApiResponse(raw);
  },
};
...
export const donationsApi = {
  create: async (
    ...
    ApiResponse<{ checkoutUrl?: string; reference: string; instructions?: string; paymentMethod?: string }>
  > => {
  ...
  getStats: async (): Promise<ApiResponse<{ total: number; count: number; average: number }>> => {
...
export const mockData = {
  ...
  stories: [
    ...
    },
    {
      ...
      excerpt: 'Recovery is not just individual, it involves the whole family...',
      content: 'Full story content here...',
