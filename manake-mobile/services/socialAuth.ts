...
export async function startGoogleAuth(): Promise<GoogleAuthResult> {
  ...
  if (result.type !== "success") {
    return {
      ok: false,
      error: result.type === "cancel" ? "Cancelled" : "Google sign-in failed",
    };
  }
...
export async function startAppleAuth(): Promise<AppleAuthResult> {
  if (!isAppleConfigured()) {
    return {
      ok: false,
      error:
        "Apple sign-in is not configured. Set EXPO_PUBLIC_APPLE_CLIENT_ID in your environment.",
    };
  }
  ...
  if (result.type !== "success") {
    return {
      ok: false,
      error: result.type === "cancel" ? "Cancelled" : "Apple sign-in failed",
    };
  }
