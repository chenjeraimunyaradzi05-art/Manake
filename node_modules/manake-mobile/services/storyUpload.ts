      ...
      const url = await uploadMedia(media.uri, media.mimeType, (progress) => {
        ...
        const adjustedProgress: UploadProgress = {
          ...
          percentage: Math.round(
            ((i / data.media!.length) * 100) + (progress.percentage / data.media!.length)
          ),
        };
 ...
 * Load a saved draft
 */
export async function loadDraft(draftId: string): Promise<StoryUploadData | null> {
...
export async function deleteDraft(draftId: string): Promise<void> {
  ...
  const draftsDir = `${FileSystem.documentDirectory}drafts/`;
  await FileSystem.deleteAsync(`${draftsDir}${draftId}.json`, { idempotent: true });
}

/**
 * Get all local drafts
 */
export async function getLocalDrafts(): Promise<{ id: string; data: StoryUploadData }[]> {
  if (!FileSystem) return [];
