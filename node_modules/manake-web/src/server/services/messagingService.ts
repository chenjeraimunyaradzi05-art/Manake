...
export async function sendUnifiedMessage(
  ...
  const { userId, channels, message, recipientPhone, recipientId, mediaUrl } = options;
  ...
  for (const channel of channels) {
    ...
    try {
      switch (channel) {
        case 'whatsapp': {
          ...
          const waResult: WhatsAppMessageResult = await sendWhatsAppMessage(waOptions);
        ...
        case 'facebook': {
          ...
          const fbResult: MessengerSendResult = await sendMessengerMessage(fbOptions);
    ...
    try {
      await Message.create({
        ...
        recipientPhone: channel === 'whatsapp' || channel === 'sms' ? recipientPhone : undefined,
        content: message,
