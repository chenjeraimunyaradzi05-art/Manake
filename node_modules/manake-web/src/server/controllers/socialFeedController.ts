...
import socialFeedService, { SocialPlatform } from '../services/socialFeedService';
...
export const getSocialFeed = async (req: Request, res: Response): Promise<void> => {
  ...
  const result = await socialFeedService.getSocialFeed({ platforms, limit, cursor });
  res.json(result);
};
...
export const getFacebookFeed = async (req: Request, res: Response): Promise<void> => {
  const limit = Number(req.query.limit) || 20;
  const cursor = req.query.cursor as string | undefined;
...
export const getTwitterFeed = async (req: Request, res: Response): Promise<void> => {
  const limit = Number(req.query.limit) || 20;
  const cursor = req.query.cursor as string | undefined;
...
export const likeSocialPost = async (req: Request, res: Response): Promise<void> => {
  const { platform, postId } = req.params as { platform: SocialPlatform; postId: string };
  await socialFeedService.likeSocialPost(postId, platform);
  res.json({ message: 'Liked' });
};

export const unlikeSocialPost = async (req: Request, res: Response): Promise<void> => {
  const { platform, postId } = req.params as { platform: SocialPlatform; postId: string };
  await socialFeedService.unlikeSocialPost(postId, platform);
  res.json({ message: 'Unliked' });
};

export const shareSocialPost = async (req: Request, res: Response): Promise<void> => {
  const { platform, postId } = req.params as { platform: SocialPlatform; postId: string };
