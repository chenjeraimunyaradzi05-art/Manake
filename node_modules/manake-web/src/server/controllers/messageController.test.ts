...
vi.mock('../models/Message', () => ({
  Message: {
    ...
    findOneAndUpdate: vi.fn(async () => ({ ...mockMessage, status: 'read', readAt: new Date() })),
  },
}));
...
describe('messageController', () => {
  ...
  it('creates a message', async () => {
    ...
    const payload = (res as Response & { body: unknown }).body as { data: { id: string } };
  ...
  it('throws when updating missing message', async () => {
    ...
    const req = makeReq({ params: { id: 'missing' }, body: { status: 'sent' } });
    const res = makeRes();

    await expect(updateMessageStatus(req, res)).rejects.toBeInstanceOf(NotFoundError);
  });
