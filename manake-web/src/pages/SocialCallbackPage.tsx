...
export const SocialCallbackPage = () => {
  ...
  const provider = useMemo(() => (isSupportedProvider(rawProvider) ? rawProvider : null), [rawProvider]);
  const [searchParams] = useSearchParams();
  const code = searchParams.get('code');
  const state = searchParams.get('state') || undefined;
  const mode = (searchParams.get('mode') as 'login' | 'link' | null) || undefined;
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const stored = storedRaw ? (JSON.parse(storedRaw) as { state?: string; redirectUri?: string }) : null;
      ...
      const redirectUri = stored?.redirectUri || `${window.location.origin}/auth/${provider}/callback`;

      try {
        setMessage('Finishing authentication...');
        const response = await completeSocialAuth(provider, { code, state, redirectUri, mode });
        handleAuthSuccess({
          ...
          user: {
            ...
            avatar: (response.user as { avatar?: string | null } | undefined)?.avatar ?? null,
          },
        });
    ...
    run();
  }, [code, mode, navigate, provider, state]);
  ...
  return (
    <div className="container-custom py-20 flex items-center justify-center">
      <div className="bg-white border border-gray-100 shadow-lg rounded-2xl p-8 w-full max-w-lg text-center">
        {status === 'pending' ? <Loader2 className="mx-auto animate-spin text-primary-600" size={32} /> : null}
        {status === 'success' ? <CheckCircle className="mx-auto text-green-600" size={32} /> : null}
        {status === 'error' ? <AlertCircle className="mx-auto text-red-600" size={32} /> : null}

        <h1 className="text-xl font-bold text-gray-900 mt-4">
          {status === 'success' ? 'Welcome back!' : status === 'error' ? 'Sign in failed' : 'Signing you in...'}
        </h1>
