...
const resolveSource = (provider: string): WebhookSource => {
  if (['instagram', 'facebook', 'whatsapp', 'stripe', 'google'].includes(provider)) {
...
export const ingestWebhook = async (req: Request, res: Response): Promise<void> => {
  ...
  const eventId = (req.headers['x-event-id'] || req.headers['x-request-id']) as string | undefined;
  const eventType = (req.headers['x-event-type'] || req.headers['x-message-type']) as string | undefined;
