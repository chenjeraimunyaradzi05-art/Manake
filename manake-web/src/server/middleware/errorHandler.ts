 ...
 */
function handleMongoError(error: Error & { code?: number; keyPattern?: Record<string, unknown> }): ApiError {
  // Duplicate key error
...
export const errorHandler = (
  ...
  if (isApiError(err)) {
    apiError = err;
  } else if (err.name === 'MongoError' || err.name === 'ValidationError' || err.name === 'CastError') {
    apiError = handleMongoError(err as Error & { code?: number; keyPattern?: Record<string, unknown> });
  } else if (err.name === 'JsonWebTokenError' || err.name === 'TokenExpiredError') {
    apiError = handleJWTError(err);
  } else if (err.name === 'SyntaxError' && 'body' in err) {
    // JSON parse error
    apiError = new ApiError('Invalid JSON in request body', 400, 'INVALID_JSON');
  } else {
  ...
  if (apiError.code === 'RATE_LIMIT_EXCEEDED') {
    const retryAfter = (apiError as ApiError & { retryAfter?: number }).retryAfter || 60;
