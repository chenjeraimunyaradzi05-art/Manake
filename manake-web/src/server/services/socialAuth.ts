...
import {
  OAuth2Client,
  TokenPayload as GoogleIdTokenPayload,
...
function requireAppleConfig(): {
  ...
  if (!appleClientId)
    throw new BadRequestError("APPLE_CLIENT_ID must be configured");
  if (!appleTeamId)
    throw new BadRequestError("APPLE_TEAM_ID must be configured");
  if (!appleKeyId) throw new BadRequestError("APPLE_KEY_ID must be configured");
  if (!applePrivateKey)
    throw new BadRequestError("APPLE_PRIVATE_KEY must be configured");
...
function requireRedirectUri(provided?: string): string {
  ...
  if (!uri) {
    throw new BadRequestError(
      "Redirect URI is required (set SOCIAL_REDIRECT_URI or pass redirectUri)",
    );
  }
  return uri;
}
...
export function buildAuthUrl(
  ...
  if (provider === "apple") {
    ...
    throw new BadRequestError(
      "Apple authorize URL is not supported via this endpoint",
    );
  }
